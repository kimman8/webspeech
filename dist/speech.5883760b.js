...
function handleResult({
  ...
  if (!(0, _colors.isValidColor)(color)) return; // const colorSpan = document.querySelector(`.${color}`);

  const colorSpan = document.getElementsByClassName(color)[0];
  console.log(colorSpan);
  console.log('it i s yay');
  console.log(color);
  colorSpan.classList.add('got'); //change da background color

  document.body.style.backgroundColor = color; //if it is then show da UI for that
...
function displayColors(colors) {
  return colors.map(color => `<span class="color ${(0, _colors.isDark)(color) ? 'dark' : ''} ${color}" style="background:${color};" >${color}</span>`).join('');
}
